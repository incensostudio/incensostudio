...
(() => {
  ...
  const clientPicker = (opts) => {
    ...
    const draw = () => {
      ...
      const list = () => { const q = st.cq.trim().toLowerCase(); res.innerHTML = ''; if (!q) return; const L = M.db.clients.filter((x) => (x.name + ' ' + x.phone).toLowerCase().includes(q)).sort((x, y) => x.name.localeCompare(y.name)).slice(0, 6); if (!L.length) { res.innerHTML = '<p class="bkf-note">No client with that name or number.</p>'; return; } L.forEach((x) => { const r = el('<button type="button" class="bkf-res">' + clientAvatar(x, 'lg') + '<span class="b"><b>' + esc(x.name) + '</b><small>' + esc(x.phone) + '</small></span>' + icon('chev') + '</button>'); r.onclick = () => { st.clientId = x.id; st.cq = ''; draw(); fire(); }; res.appendChild(r); }); };
