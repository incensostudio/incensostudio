...
(() => {
  ...
  const calendar = (d) => {
    ...
    staff.forEach((s) => {
      ...
      col.onclick = (e) => { if (e.target !== col || M.myStaff()) return; const y = e.offsetY; const t = open + Math.floor(y / (HOUR_H / 2)) / 2; M.newBooking({ day: d, staff: s.name, time: t }); };
