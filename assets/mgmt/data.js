...
(() => {
  ...
  const FLAGS = { ownOnly: 'Only their own chair', settle: 'Settle visits & take payments', refunds: 'Cancel, refund & release', prices: 'Change prices & stock', amounts: 'See money amounts', contacts: 'See client phone numbers', broadcast: 'Send messages to clients', access: 'Manage who can sign in' };
  const PRESETS = {
    owner: { label: 'Owner', desc: 'Everything — calendar, clients, sales, finances, reports, settings, and who can sign in.', modules: MODULES.slice(), flags: { ownOnly: false, settle: true, refunds: true, prices: true, amounts: true, contacts: true, broadcast: true, access: true } },
    desk: { label: 'Reception', desc: 'Runs the desk — all chairs on the calendar, book and settle visits, take payments, clients, shop orders, gift cards, stock and messages. No finances, reports, refunds or settings.', modules: ['home', 'today', 'bookings', 'clients', 'orders', 'gifts', 'products', 'supply', 'messages'], flags: { ownOnly: false, settle: true, refunds: false, prices: false, amounts: true, contacts: true, broadcast: false, access: false } },
    stylist: { label: 'Stylist', desc: 'Their own chair only — today\u2019s bookings, client notes and preferences. No prices, payments or settings.', modules: ['home', 'today', 'bookings', 'clients'], flags: { ownOnly: true, settle: false, refunds: false, prices: false, amounts: false, contacts: false, broadcast: false, access: false } },
  };
  ...
  // The website stores staff as a label ("Peru", or multi-chair "Peru (Hair) · First available (Nails)").
  // The desk calendar slots by a single chair name — use the first named chair for placement.
  const primaryChair = (s) => { if (!s) return ''; const first = String(s).split('·')[0].trim(); return first.replace(/\s*\([^)]*\)\s*$/, '').trim() || first; };
  ...
  // Orders in the website's vocabulary: status is a label ("Ready for pickup"), method is fulfilment
  // ("Studio pickup" / "Shipped") and pay is the payment label. The desk works in short codes.
  const orCode = (l) => { const s = String(l || '').toLowerCase(); if (/cancel/.test(s)) return 'cancelled'; if (/collect/.test(s)) return 'collected'; if (/deliver/.test(s)) return 'delivered'; if (/courier|shipped|on its way|with-courier/.test(s)) return 'with-courier'; if (/ready/.test(s)) return 'ready'; if (/prepar/.test(s)) return 'preparing'; return 'placed'; };
  const OR_LBL = { placed: 'Placed', preparing: 'Preparing', ready: 'Ready for pickup', 'with-courier': 'With courier', delivered: 'Delivered', collected: 'Collected', cancelled: 'Cancelled' };
  const orLabel = (o) => { const c = o.status || 'placed'; if (o._statusLabel && orCode(o._statusLabel) === c) return o._statusLabel; if (c === 'placed' && o.payStatus === 'pending') return 'Awaiting payment'; if (c === 'preparing' && o.payStatus === 'paid' && o.fulfil === 'delivery') return 'Paid · preparing'; return OR_LBL[c] || c; };
  // Order lines: the website stores { name, q, price }; the desk works with { productId, name, qty, price }.
  const orItemIn = (i) => { const qty = i.qty != null ? i.qty : (i.q != null ? i.q : 1); const p = i.productId ? null : (db.products || []).find((x) => (x.name || '').trim().toLowerCase() === (i.name || '').trim().toLowerCase()); const o = Object.assign({}, i, { qty }); delete o.q; if (p) o.productId = p.id; return o; };
  const orItemOut = (i) => { const o = { name: i.name, q: i.qty != null ? i.qty : (i.q != null ? i.q : 1), price: i.price }; Object.keys(i).forEach((k) => { if (!['name', 'q', 'qty', 'price'].includes(k) && i[k] !== undefined) o[k] = i[k]; }); return o; };
  const fromOr = (r) => ({ ref: r.ref, clientId: r.user_id || null, name: r.name, phone: r.phone, email: r.email, items: (r.items || []).map(orItemIn), total: r.total, method: payCode(r.pay || (/shipped|pickup/i.test(r.method || '') ? '' : r.method)), status: orCode(r.status), _statusLabel: r.status, payStatus: r.pay_status, fulfil: r.fulfil || (/shipped/i.test(r.method || '') ? 'delivery' : /pickup/i.test(r.method || '') ? 'pickup' : ((r.address && (r.address.text || r.address.line1)) ? 'delivery' : 'pickup')), delivery: r.delivery || 0, address: typeof r.address === 'string' ? r.address : ((r.address && (r.address.text || [r.address.line1, r.address.city].filter(Boolean).join(', '))) || ''), discount: r.discount || 0, tier: r.tier, gift: (r.gift && r.gift.amount) ? r.gift : null, giftParts: (r.gift && r.gift.parts) || r.gift_parts || [], courier: r.courier || null, source: r.source || 'web', soldBy: r.sold_by, refund: r.refund || 0, refundSent: r.refund_sent, partial: r.partial, cardLink: r.card_link, placedAt: r.placed_at ? new Date(r.placed_at).getTime() : undefined, deadline: r.deadline || r.pay_deadline, cancelReason: r.cancel_reason, cancelledAt: r.cancelled_at, notes: r.notes, toPay: r.to_pay });
  const toOr = (o) => ({ ref: o.ref, user_id: (o.clientId && profileIds.has(o.clientId)) ? o.clientId : null, items: (o.items || []).map(orItemOut), total: o.total || 0, method: o.fulfil === 'delivery' ? 'Shipped' : 'Studio pickup', pay: PAY_LBL[o.method] || o.pay || null, name: o.name || null, phone: o.phone || null, email: o.email || null, address: typeof o.address === 'string' ? { text: o.address } : (o.address || {}), status: orLabel(o), pay_status: o.payStatus || null, discount: o.discount || 0, tier: o.tier || null, gift: o.gift || null, gift_parts: o.giftParts || [], courier: o.courier || null, source: o.source || null, sold_by: o.soldBy || null, refund: o.refund || 0, refund_sent: !!o.refundSent, partial: !!o.partial, fulfil: o.fulfil || null, delivery: o.delivery || 0, card_link: o.cardLink || null, deadline: o.deadline || null, cancelled_at: o.cancelledAt || null, cancel_reason: o.cancelReason || null, notes: o.notes || null, to_pay: (o.toPay != null ? o.toPay : null), placed_at: o.placedAt ? new Date(o.placedAt).toISOString() : null });
  ...
  // Write the website's vocabulary: note (one-liner), details (long copy), lowercase cat.
  const toProd = (p) => { if (!UUID.test(p.id || '')) p.id = uuid(); const imgs = (p.images && p.images.length) ? p.images.filter(Boolean) : (p.image ? [p.image] : []); return { id: p.id, name: p.name, price: p.price || 0, cost: p.cost || 0, stock: p.stock || 0, low: p.low || 0, image_url: imgs[0] || null, images: imgs, cat: (p.category || '').toLowerCase() || null, category: p.category || null, note: p.note || null, details: p.desc || null, descr: p.desc || null, sku: p.sku || null, active: p.active !== false }; };

  const fromStaff = (r) => ({ name: r.name, cats: r.cats || [], role: r.role, accent: r.accent, bio: r.bio || '', photo: r.photo_url || null, phone: r.phone || '', commission: r.commission != null ? Number(r.commission) : 0, days: (r.days && r.days.length) ? r.days : [0, 1, 2, 3, 4, 5, 6], start: r.start_hour != null ? Number(r.start_hour) : 10, end: r.end_hour != null ? Number(r.end_hour) : 19, timeOff: r.time_off || [], services: r.services || [], active: r.active !== false });
  ...
  const hydrate = async () => {
    ...
    db.clients = clients;
    db.products = wp.map(fromProd);   // before orders, so order lines can be matched to products
    ...
    // The site's web_config.hours can omit closedDays/closedDates; keep the shapes the app relies on.
    db.settings.hours = Object.assign({ open: 10, closeWeek: 19, closeWeekend: 20, closedDays: [] }, db.settings.hours || {});
    if (!Array.isArray(db.settings.closedDates)) db.settings.closedDates = [];
