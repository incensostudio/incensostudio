...
(() => {
  ...
  const newBooking = (o) => {
    ...
    const saveEdit = (segs, from) => { saved = true;
      const rate = disc().rate; const visitRef = E0.visit || E0.ref; const prefs = { mood: st.mood, flags: st.flags.slice(), smoke: st.smoke }; const c = client(); if (c) c.prefs = prefs;
    ...
    foot.querySelector('[data-o]').onclick = async () => {
      let c = E ? client() : picker.commit(); if (!c && !E) return; if (c) st.clientId = c.id;
