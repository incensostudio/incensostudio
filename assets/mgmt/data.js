...
(() => {
  ...
  const fromOr = (r) => ({ ref: r.ref, clientId: r.user_id || null, name: r.name, phone: r.phone, email: r.email, items: r.items || [], total: r.total, method: payCode(r.pay || (/shipped|pickup/i.test(r.method || '') ? '' : r.method)), status: orCode(r.status), _statusLabel: r.status, payStatus: r.pay_status, fulfil: r.fulfil || (/shipped/i.test(r.method || '') ? 'delivery' : /pickup/i.test(r.method || '') ? 'pickup' : ((r.address && (r.address.text || r.address.line1)) ? 'delivery' : 'pickup')), delivery: r.delivery || 0, address: (r.address && (r.address.text || r.address)) || '', discount: r.discount || 0, tier: r.tier, gift: (r.gift && r.gift.amount) ? r.gift : null, giftParts: (r.gift && r.gift.parts) || r.gift_parts || [], courier: r.courier || null, source: r.source || 'web', soldBy: r.sold_by, refund: r.refund || 0, refundSent: r.refund_sent, partial: r.partial, cardLink: r.card_link, placedAt: r.placed_at ? new Date(r.placed_at).getTime() : undefined, deadline: r.deadline || r.pay_deadline, cancelReason: r.cancel_reason, cancelledAt: r.cancelled_at, notes: r.notes, toPay: r.to_pay });
  const toOr = (o) => ({ ref: o.ref, user_id: (o.clientId && profileIds.has(o.clientId)) ? o.clientId : null, items: o.items || [], total: o.total || 0, method: o.fulfil === 'delivery' ? 'Shipped' : 'Studio pickup', pay: PAY_LBL[o.method] || o.pay || null, name: o.name || null, phone: o.phone || null, email: o.email || null, address: typeof o.address === 'string' ? { text: o.address } : (o.address || {}), status: orLabel(o), pay_status: o.payStatus || null, discount: o.discount || 0, tier: o.tier || null, gift: o.gift || null, gift_parts: o.giftParts || [], courier: o.courier || null, source: o.source || null, sold_by: o.soldBy || null, refund: o.refund || 0, refund_sent: !!o.refundSent, partial: !!o.partial, fulfil: o.fulfil || null, delivery: o.delivery || 0, card_link: o.cardLink || null, deadline: o.deadline || null, cancelled_at: o.cancelledAt || null, cancel_reason: o.cancelReason || null, notes: o.notes || null, to_pay: (o.toPay != null ? o.toPay : null), placed_at: o.placedAt ? new Date(o.placedAt).toISOString() : null });
